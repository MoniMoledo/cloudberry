...
public class Tweet {
    ...
    public static String toBrADM(Status status, BrGeoGnosis brGnosis, boolean requireGeoField) throws UnknownPlaceException{
        String geoTags = brGeoTag(status, brGnosis, requireGeoField);
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        ADM.keyValueToSbWithComma(sb, CREATE_AT, ADM.mkDateTimeConstructor(status.getCreatedAt()));
        ADM.keyValueToSbWithComma(sb, ID, ADM.mkInt64Constructor(status.getId()));
    ...
    public static String brGeoTag(Status status, BrGeoGnosis gnosis, boolean requireGeoField) throws UnknownPlaceException {
        StringBuilder sb = new StringBuilder();
        if (brTextMatchPlace(sb, status, gnosis)) {
            return sb.toString();
        }
        GeoLocation location = status.getGeoLocation();
        if (brExactPointLookup(sb, location, gnosis)) {
            return sb.toString();
        }
        if (requireGeoField) {
    ...
    protected static boolean brExactPointLookup(StringBuilder sb, GeoLocation location, BrGeoGnosis gnosis) {
        if (location == null) {
            return false;
        }

        scala.Option<BrGeoGnosis.BrGeoTagInfo> info = gnosis.tagPoint(location.getLongitude(), location.getLatitude());
        if (info.isEmpty()) {
            return false;
        }
    ...
    protected static boolean brTextMatchPlace(StringBuilder sb, Status status, BrGeoGnosis gnosis) {
        twitter4j.Place place = status.getPlace();
        if (place == null) {
            return false;
        }
        String country = place.getCountry();
        if ((!("Brasil").equals(country)) && (!("Brazil").equals(country))) {
            return false;
        }
        scala.Option<BrGeoGnosis.BrGeoTagInfo> info;
        ...
        switch (type) {
            ...
            case "city":
                ...
                //String stateAbbr = place.getFullName().substring(index + 1).trim();  Most of the cases == Brasil, not the state
                String cityName = place.getName();
                info = gnosis.tagCity(cityName);
                break;
    ...
    protected static boolean textMatchPlace(StringBuilder sb, Status status, USGeoGnosis gnosis) {
        ...
        switch (type) {
            ...
            case "city":
                ...
                String stateAbbr = place.getFullName().substring(index + 1).trim(); //most of the times == Brasil -> not useful

                String cityName = null;
                try {
                    cityName = new String(place.getName().getBytes("UTF-8"), "ISO-8859-1");
