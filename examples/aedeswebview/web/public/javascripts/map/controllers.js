angular.module('cloudberry.map', ['leaflet-directive', 'cloudberry.common','cloudberry.cache'])
  .controller('MapCtrl', function($scope, $window, $http, $compile, cloudberry, leafletData, cloudberryConfig, Cache) {
    ...
    $scope.$watchCollection(
      function() {
        return {
          'tmapResult': cloudberry.tweetMapResult,
          'nmapResult': cloudberry.newsMapResult,
          'totalCount': cloudberry.newsCount + cloudberry.tweetCount,
      ...
      function(newResult, oldValue) {
        ...
        if(!newsChanged && twitterChanged){
          $scope.result = newResult['tmapResult'].concat(oldValue['nmapResult']);
        }
